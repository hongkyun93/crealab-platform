...
export async function GET(request: Request) {
    ...
    try {
        ...
        // instagram_business_account = 비즈니스 계정
        // connected_instagram_account = 크리에이터 계정 (비즈니스도 포함)
        const pagesRes = await fetch(
            `https://graph.facebook.com/v19.0/me/accounts` +
            `?access_token=${accessToken}&fields=id,name,access_token,instagram_business_account,connected_instagram_account`
        )
        ...
        const me2Res = await fetch(
            ...
            `?fields=accounts{id,name,access_token,instagram_business_account{id,username,followers_count},connected_instagram_account{id,username,followers_count}}&access_token=${accessToken}`
        )
        ...
        if (!igUserId) {
            ...
            const directRes = await fetch(
                `https://graph.facebook.com/v19.0/me?fields=id,name,instagram_business_account,connected_instagram_account` +
                `&access_token=${accessToken}`
            )
            const directData = await directRes.json()
            console.log('[Instagram OAuth] direct me:', JSON.stringify(directData, null, 2))
            const directIg = directData.instagram_business_account || directData.connected_instagram_account
            if (directIg?.id) {
                igUserId = directIg.id
            }
        }
        ...
        if (!igUserId) {
            ...
            for (const biz of (bizData.data || [])) {
                ...
                const clientRes = await fetch(
                    ...
                    `?fields=id,name,access_token,instagram_business_account{id,username,followers_count},connected_instagram_account{id,username,followers_count}` +
                    `&access_token=${accessToken}`
                )
