...
const MCN_FEATURES = [
    ...
    { icon: BarChart3, title: "광고 성과 리포트", desc: "크리에이터별 CPE·참여율·완료 협업 수를 자동 집계합니다. 브랜드 재계약 협상 시 데이터로 바로 제출하세요.", highlight: "성과는 숫자, 통계 데이터로 말해요" },
    ...
    { icon: Eye, title: "프록시 모드", desc: "소속 크리에이터를 대리하여 모먼트 등록, 계약 서명, 배송 확인까지 직접 처리합니다. 크리에이터 연락을 기다릴 필요가 없습니다.", highlight: "크리에이터 연락 기다릴 필요 X" },
                    ...
                    <div className="space-y-8">
                        <div className={`p-6 sm:p-8 md:p-10 rounded-3xl bg-gradient-to-br ${currentTab.color} text-white`}>
                            <div className="max-w-none">
                                ...
                                <p className="text-white/80 leading-relaxed">{activeTab === "creator" ? "나에게 딱 필요한 광고 영업·계약·정산은 CreadyPick이 처리합니다. 일정을 등록하고, 라이프 모먼트에 필요한 제품을 광고하세요. 광고주님은 CreadyPick이 찾아드립니다!" : activeTab === "mcn" ? "크리에이터가 10명이든 100명이든 — 현황 파악, 일정 관리, 제안 관리, 성과 분석, 컨택, 계약, 정산을 한곳에서 한번에!" : "광고주님 너무 바쁘시죠? 광고에 딱 맞는 인플루언서 서치·컨택·계약·정산까지 CreadyPick에 맡기고, 본업에만 집중하세요!"}</p>
                            </div>
                        </div>
