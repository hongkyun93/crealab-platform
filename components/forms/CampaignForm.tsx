...
export function CampaignForm({ mode, campaignId }: CampaignFormProps) {
    ...
    const { refreshData, products, user, currentTeam, campaigns } = useUnifiedProvider()
    ...
    useEffect(() => {
        if (mode === 'edit' && campaignId && campaigns.length > 0) {
            const campaign = campaigns.find(c => c.id.toString() === campaignId || c.id === campaignId)
                    ...
                    <div className="mb-4 flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold tracking-tight">
                                {mode === 'create' ? '새 캠페인 등록하기' : '캠페인 수정하기'}
                            </h1>
