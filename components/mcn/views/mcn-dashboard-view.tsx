...
import {
    AlertTriangle, ArrowUpDown, BarChart3, Calendar, ChevronRight,
    FileText, LayoutGrid, Search, Table as TableIcon, Users
} from "lucide-react"
import { CreatorSummaryCard, type CreatorStatus } from "../creator-summary-card"
import type { CreatorSummary, FilterStatus, SortOrder, ViewMode, PriceRange } from "../types/mcn"
...
interface McnDashboardViewProps {
    summaryData: CreatorSummary[]
    filteredCreators: (CreatorSummary & { _status: CreatorStatus })[]
    aggregateStats: AggregateStats
    urgentCreators: CreatorSummary[]
    idleCreators: CreatorSummary[]
    allTags: string[]
    // Filter state
    searchQuery: string
    ...
    onViewCreator: (userId: string) => void
}
...
// ─── Component ────────────────────────────────────────────────────────────────
export function McnDashboardView({
    summaryData, filteredCreators, aggregateStats, urgentCreators, idleCreators,
    allTags, searchQuery, filterStatus, sortOrder, viewMode, priceRange, tagFilter,
    setSearchQuery, setFilterStatus, setSortOrder, setViewMode, setPriceRange, setTagFilter,
    onViewCreator,
}: McnDashboardViewProps) {
            ...
            <div>
                <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
                    ...
                    <div className="flex items-center gap-1 bg-muted p-1 rounded-lg">
                        <Button variant={viewMode === 'grid' ? 'default' : 'ghost'} size="icon" className="h-7 w-7" onClick={() => setViewMode('grid')} title="카드형">
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                    <div className="relative flex-1 min-w-[200px]">
                        ...
                        <Input placeholder="이름, 인스타, 태그 검색..." className="pl-9 h-9" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
                    </div>
                    <Select value={sortOrder} onValueChange={v => setSortOrder(v as SortOrder)}>
                        <SelectTrigger className="h-9 w-[140px]">
                            <ArrowUpDown className="h-3.5 w-3.5 mr-1.5 text-muted-foreground" /><SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <Select value={priceRange} onValueChange={v => setPriceRange(v as PriceRange)}>
                        <SelectTrigger className="h-9 w-[130px]"><SelectValue placeholder="단가 범위" /></SelectTrigger>
                        <SelectContent>
                    ...
                    {allTags.length > 0 && (
                        <Select value={tagFilter} onValueChange={setTagFilter}>
                            <SelectTrigger className="h-9 w-[120px]"><SelectValue placeholder="태그" /></SelectTrigger>
                            <SelectContent>
                    ...
                    {FILTER_CHIPS.map(chip => (
                        ...
                        >
                            ...
                            {chip.value === 'urgent' && urgentCreators.length > 0 && (
                                <span className="ml-1.5 bg-red-500 text-white text-[9px] font-bold rounded-full px-1.5 py-0.5">{urgentCreators.length}</span>
                            )}
                            {chip.value === 'idle' && idleCreators.length > 0 && (
                                <span className="ml-1.5 bg-slate-400 text-white text-[9px] font-bold rounded-full px-1.5 py-0.5">{idleCreators.length}</span>
                            )}
                        </button>
                    ))}
                    <span className="text-xs text-muted-foreground ml-1">{filteredCreators.length}/{summaryData.length}명</span>
                </div>

                {/* Creator Grid / Table / Empty */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {filteredCreators.map(creator => (
                            <CreatorSummaryCard
                                ...
                                onViewDashboard={() => onViewCreator(creator.user_id)}
                            />
                        ))}
                    </div>
                ) : (
                                ...
                                {filteredCreators.map(creator => {
                                    ...
                                    return (
                                        <tr
                                            ...
                                            onClick={() => onViewCreator(creator.user_id)}
                                        >
