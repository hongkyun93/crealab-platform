...
import { BarChart3, Bookmark, ChevronDown, Eye, Film, Heart, Image as ImageIcon, Instagram, Lightbulb, Loader2, MessageCircle, Share2, Sparkles, TrendingUp, X } from "lucide-react"
...
const SLOTS = [
    { key: "account" as SlotKey, label: "계정 인사이트", description: "프로필 → 프로페셔널 대시보드 → 계정 인사이트", icon: BarChart3, badgeItems: ["팔로워 수", "도달한 계정", "참여 계정"] },
    { key: "post" as SlotKey, label: "게시물 인사이트", description: "게시물 → 인사이트 보기", icon: ImageIcon, badgeItems: ["좋아요", "댓글", "저장", "도달 수"] },
    { key: "reels" as SlotKey, label: "릴스 인사이트", description: "릴스 → 인사이트 보기", icon: Film, badgeItems: ["재생 수", "좋아요", "저장", "평균 시청"], recommended: true },
]
...
function Section({ title, children, className }: { title: string; children: React.ReactNode; className?: string }) {
    return (
        <div className={cn("rounded-xl overflow-hidden border border-stone-200 dark:border-neutral-700", className)}>
...
export default function InsightAnalyzer() {
    const { channels, fetchChannels } = useSocialChannels()
            ...
            <div className="flex items-center justify-between mb-3 shrink-0">
                <div>
                    ...
                    <p className="text-xs text-stone-400 dark:text-neutral-500 mt-0.5">
                        소셜 채널 연결 시 실측 데이터로 자동 계산
                    </p>
                </div>
            </div>

            {/* ── 2-Column Layout ── */}
            <div className="flex flex-col md:flex-row gap-4 flex-1 md:h-[calc(100vh-180px)] overflow-hidden">

                {/* ── LEFT: Tabs (소셜채널 | 캡처분석) ── */}
                <div className="w-full md:w-[44%] md:overflow-y-auto scrollbar-thin">
                    <Tabs defaultValue="channel" className="space-y-3">
                        ...
                        {/* ── Tab: 소셜채널 ── */}
                        <TabsContent value="channel" className="space-y-3 mt-0">
                            <Section title="연결된 채널">
                                {channels && channels.length > 0 ? (
                                            ...
                                            return (
                                                <div key={ch.id} className={cn("flex items-center justify-between px-4 py-3 bg-stone-50 dark:bg-neutral-900", !isLast && "border-b border-stone-100 dark:border-neutral-800")}>
                                                    <div className="flex items-center gap-2.5">
                                                        <div className={cn("w-1.5 h-1.5 rounded-full shrink-0", igStats?.source === 'instagram_api' && ch.platform === 'instagram' ? 'bg-emerald-400' : 'bg-stone-300 dark:bg-neutral-600')} />
                                                        <div>
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        {ch.platform === 'instagram' && igStats?.source === 'instagram_api' && (
                                                            <span className="text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 dark:bg-emerald-950 dark:border-emerald-800 dark:text-emerald-400">API</span>
                                                        )}
                                                        <span className="text-xs font-semibold text-stone-800 dark:text-neutral-200">
                                                            {ch.followersCount ? `${(ch.followersCount / 10000).toFixed(1)}만` : '—'}
                                                        </span>
                                                    </div>
                                                </div>
                                            )
                                    ...
                                    <div className="px-4 py-5 bg-stone-50 dark:bg-neutral-900 text-center">
                                        <p className="text-xs text-stone-400 dark:text-neutral-600">연결된 채널이 없습니다</p>
                                    </div>
                                )}
                                {/* IG Connect */}
                                {effectiveUserId && (
                                    <div className="px-4 py-3 border-t border-stone-200 dark:border-neutral-700 bg-stone-100 dark:bg-neutral-800">
                                        <DropdownMenu>
                            ...
                            {/* IG 실측 인사이트 */}
                            {igStatsLoading && (
                                <div className="flex items-center gap-2 px-4 py-3 rounded-xl border border-stone-200 dark:border-neutral-700 bg-stone-50 dark:bg-neutral-900">
                        ...
                        <TabsContent value="capture" className="space-y-3 mt-0">
                            {igStats?.source === 'instagram_api' && (
                                <div className="px-3 py-2.5 rounded-xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/20 text-xs text-emerald-700 dark:text-emerald-400">
                                    API 연결로 실측 데이터가 있습니다. 캡처 분석은 참고용입니다.
                                </div>
                            )}
                            {SLOTS.map(slot => {
                                ...
                                return (
                                    <div key={slot.key} className={cn("rounded-xl border overflow-hidden", state.result ? 'border-emerald-200 dark:border-emerald-800' : 'border-stone-200 dark:border-neutral-700')}>
                                        <div className={cn("px-4 py-2 flex items-center justify-between border-b", state.result ? 'bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-800' : 'bg-stone-100 dark:bg-neutral-800 border-stone-200 dark:border-neutral-700')}>
                                            <div className="flex items-center gap-2">
                                                <span className="text-xs font-semibold text-stone-700 dark:text-neutral-300">{slot.label}</span>
                                                {slot.recommended && <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-amber-100 dark:bg-amber-950 border border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-400 font-medium">추천</span>}
                                                {state.result && <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 font-medium">✓ 완료</span>}
                                            </div>
                                            {state.result && <span className="text-sm font-bold text-stone-700 dark:text-neutral-300">{state.result.engagementRate}% {state.result.engagementEmoji}</span>}
                                        </div>
                                        <div className="p-3 bg-stone-50 dark:bg-neutral-900">
                                            {!state.preview ? (
                                                <div className="border border-dashed border-stone-300 dark:border-neutral-700 rounded-lg p-3 text-center cursor-pointer hover:border-stone-400 dark:hover:border-neutral-600 transition-colors"
                                                    onClick={() => fileInputRefs.current[slot.key]?.click()}>
                                                    <SlotIcon className="mx-auto h-5 w-5 text-stone-300 dark:text-neutral-600 mb-1.5" />
                                                    <p className="text-[10px] text-stone-400 dark:text-neutral-600 mb-1.5">{slot.description}</p>
                                                ...
                                                <div className="space-y-2">
                                                    <div className="flex gap-3 items-center">
                                                        <div className="relative shrink-0">
                                                            <img src={state.preview!} alt={slot.label} className="w-14 h-auto rounded-lg border border-stone-200 dark:border-neutral-700 object-contain max-h-24" />
                                                            <button onClick={() => clearSlot(slot.key)} className="absolute -top-1.5 -right-1.5 bg-red-500 text-white rounded-full p-0.5">
                                                                <X className="h-3 w-3" />
                                                            </button>
                                                        </div>
                                                        <div className="flex-1 min-w-0">
                                                            <p className="text-xs font-medium text-stone-700 dark:text-neutral-300 truncate">{state.file?.name}</p>
                                                            <p className="text-[10px] text-stone-400">{state.file ? `${(state.file.size / 1024).toFixed(0)}KB` : ""}</p>
                                                            {!state.result && (
                                                                <button onClick={() => handleAnalyze(slot.key)} disabled={state.isAnalyzing}
                                                                    className="mt-1.5 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-stone-900 dark:bg-neutral-100 text-white dark:text-neutral-900 hover:bg-stone-800 disabled:opacity-50 transition-colors">
                                                    ...
                                                    {state.result && (
                                                        <div className="grid grid-cols-2 gap-1">
                                                            {[
                                                                ...
                                                                <div key={item.label} className="flex items-center gap-1.5 bg-stone-100 dark:bg-neutral-800 rounded-lg px-2 py-1">
                                                                    <item.icon className="h-3 w-3 text-stone-400 shrink-0" />
                                                                    <span className="text-[10px] text-stone-500 dark:text-neutral-500">{item.label}</span>
                                                                    <span className="text-[10px] font-semibold text-stone-800 dark:text-neutral-200 ml-auto">{fmtNum(item.value)}</span>
                                                                </div>
                                                            ))}
                                                        </div>
                                                    )}
                            ...
                            {hasAnyResult && (() => {
                                ...
                                return (
                                    <div className="space-y-2 animate-in fade-in-0 duration-500">
                                        <Section title="AI 분석 결과">
                                            <div className="flex items-center justify-between px-4 py-3 bg-stone-50 dark:bg-neutral-900">
                                                <div>
                                                    <p className="text-[10px] text-stone-400 dark:text-neutral-500 mb-0.5">AI 추천 단가</p>
                                                    ...
                                                    <p className="text-[10px] text-stone-400 mt-0.5">{results.length}개 캡처 · 평균 ER {avgEr.toFixed(1)}%</p>
                                                </div>
                                            </div>
                                        </Section>
                                        {totalTips.length > 0 && (
                                            <Section title="개선 팁">
                                                <div className="px-4 py-3 bg-stone-50 dark:bg-neutral-900 space-y-1">
                                                    {totalTips.map((tip, i) => (
                                                        <p key={i} className="text-xs text-stone-500 dark:text-neutral-500 flex items-start gap-1.5">
                ...
                {/* ── RIGHT: 광고단가 계산기 ── */}
                <div className="w-full md:flex-1 md:overflow-y-auto scrollbar-thin space-y-4">
                        ...
                        <>
                            ...
                            <Section title="핵심 지표">
                                {[
                                    ...
                                    { label: '참여율 (ER)', value: `${(er * 100).toFixed(1)}%`, badge: <span className={cn("text-[9px] px-1.5 py-0.5 rounded-full font-medium", erBadgeColor)}>{erSourceLabel}</span> },
                                    ...
                                    <div key={item.label} className={cn("flex items-center justify-between px-4 py-2.5 bg-stone-50 dark:bg-neutral-900", i < arr.length - 1 && "border-b border-stone-100 dark:border-neutral-800")}>
                                ...
                                <div className="rounded-xl overflow-hidden border border-stone-200 dark:border-neutral-700">
                                    {[
                                        ...
                                        <button key={key} onClick={() => set(!active)}
                                            className={cn("w-full flex items-center gap-3 px-4 py-2.5 text-left transition-all", i < arr.length - 1 && "border-b border-stone-100 dark:border-neutral-800", active ? 'bg-amber-50 dark:bg-amber-950/30' : 'bg-stone-50 dark:bg-neutral-900')}>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="text-[10px] text-stone-400 dark:text-neutral-600">{desc}</p>
                                            </div>
