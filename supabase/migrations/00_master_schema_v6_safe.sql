...
CREATE OR REPLACE FUNCTION public.accept_invitation(invitation_id uuid) RETURNS boolean
...
CREATE OR REPLACE FUNCTION public.add_team_member_direct(target_email text, target_role text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.can_access_submission_feedback(p_proposal_id uuid) RETURNS boolean
...
CREATE OR REPLACE FUNCTION public.complete_settlement(p_proposal_id text, p_proposal_type text) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.create_settlement_on_approval(p_proposal_id text, p_proposal_type text, p_brand_id uuid, p_creator_id uuid, p_gross_amount integer) RETURNS uuid
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
  v_team_id       uuid;
...
CREATE OR REPLACE FUNCTION public.enforce_single_primary_channel() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.generate_statement_number(target_team_id uuid, target_month text) RETURNS text
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
  seq integer;
...
CREATE OR REPLACE FUNCTION public.get_brand_balance(p_brand_id uuid) RETURNS integer
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.get_current_user_info() RETURNS json
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.get_invitation_by_code(code text) RETURNS TABLE(valid boolean, team_id uuid, team_name text, inviter_name text, inviter_avatar text, error_message text)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.get_my_invitations() RETURNS TABLE(id uuid, team_id uuid, team_name text, role text, created_at timestamp with time zone, inviter_name text)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.get_my_team_ids() RETURNS TABLE(team_id uuid)
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.get_team_dashboard_summary(target_team_id uuid) RETURNS json
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
...
CREATE OR REPLACE FUNCTION public.has_team_access(target_user_id uuid) RETURNS boolean
...
CREATE OR REPLACE FUNCTION public.invite_team_member(target_team_id uuid, target_email text, target_role text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.join_team_with_code(code text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.notify_brand_on_campaign_application() RETURNS trigger
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
    campaign_name TEXT;
...
CREATE OR REPLACE FUNCTION public.save_instagram_connection(p_user_id uuid, p_handle text, p_followers_count integer, p_ig_user_id text, p_ig_access_token text) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.save_instagram_connection_basic(p_user_id uuid, p_handle text, p_ig_user_id text, p_ig_access_token text) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.update_social_channels_updated_at() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
    NEW.updated_at = now();
...
CREATE OR REPLACE FUNCTION public.upsert_revenue_split(target_team_id uuid, target_creator_id uuid, new_ratio numeric) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.user_belongs_to_team(target_team_id uuid) RETURNS boolean
    LANGUAGE sql STABLE SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
  SELECT EXISTS (
...
CREATE INDEX IF NOT EXISTS idx_brand_deposits_brand_id ON public.brand_deposits USING btree (brand_id);
...
CREATE INDEX IF NOT EXISTS idx_brand_deposits_brand_id_created ON public.brand_deposits USING btree (brand_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_brand_deposits_status ON public.brand_deposits USING btree (status);
...
CREATE INDEX IF NOT EXISTS idx_brand_deposits_type ON public.brand_deposits USING btree (type);
...
CREATE INDEX IF NOT EXISTS idx_brand_products_brand_id ON public.brand_products USING btree (brand_id);
...
CREATE INDEX IF NOT EXISTS idx_brand_products_lookup ON public.brand_products USING btree (id, brand_id);
...
CREATE INDEX IF NOT EXISTS idx_brand_proposals_contract_status ON public.product_applications USING btree (contract_status);
...
CREATE INDEX IF NOT EXISTS idx_brand_proposals_payment ON public.product_applications USING btree (payment_confirmed_at, contract_status);
...
CREATE INDEX IF NOT EXISTS idx_campaign_performance_brand ON public.campaign_performance USING btree (brand_id);
...
CREATE INDEX IF NOT EXISTS idx_campaign_performance_creator ON public.campaign_performance USING btree (creator_id);
...
CREATE INDEX IF NOT EXISTS idx_campaign_performance_proposal ON public.campaign_performance USING btree (proposal_type, proposal_id);
...
CREATE INDEX IF NOT EXISTS idx_favorites_target ON public.favorites USING btree (target_id, target_type);
...
CREATE INDEX IF NOT EXISTS idx_life_moments_created_at ON public.life_moments USING btree (created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_life_moments_is_private ON public.life_moments USING btree (is_private);
...
CREATE INDEX IF NOT EXISTS idx_life_moments_status ON public.life_moments USING btree (status);
...
CREATE INDEX IF NOT EXISTS idx_messages_brand_proposal_id ON public.messages USING btree (product_application_id);
...
CREATE INDEX IF NOT EXISTS idx_moment_proposals_brand_team_id ON public.moment_proposals USING btree (brand_team_id);
...
CREATE INDEX IF NOT EXISTS idx_moment_proposals_payment ON public.moment_proposals USING btree (payment_confirmed_at, contract_status);
...
CREATE INDEX IF NOT EXISTS idx_profiles_lookup ON public.profiles USING btree (id);
...
CREATE INDEX IF NOT EXISTS idx_social_channels_ig_user_id ON public.social_channels USING btree (ig_user_id) WHERE (ig_user_id IS NOT NULL);
...
CREATE INDEX IF NOT EXISTS idx_social_channels_is_primary ON public.social_channels USING btree (user_id, is_primary);
...
CREATE INDEX IF NOT EXISTS idx_social_channels_platform ON public.social_channels USING btree (platform);
...
CREATE INDEX IF NOT EXISTS idx_social_channels_user_id ON public.social_channels USING btree (user_id);
...
CREATE INDEX IF NOT EXISTS idx_team_invitations_code ON public.team_invitations USING btree (invite_code);
...
CREATE INDEX IF NOT EXISTS idx_team_invitations_invite_code ON public.team_invitations USING btree (invite_code);
...
CREATE INDEX IF NOT EXISTS workspace_files_brand_proposal_id_idx ON public.workspace_files USING btree (brand_proposal_id);
...
CREATE INDEX IF NOT EXISTS workspace_files_moment_proposal_id_idx ON public.workspace_files USING btree (moment_proposal_id);
...
CREATE INDEX IF NOT EXISTS workspace_files_proposal_id_idx ON public.workspace_files USING btree (proposal_id);
...
CREATE INDEX IF NOT EXISTS workspace_files_workspace_id_idx ON public.workspace_files USING btree (workspace_id);
...
DROP TRIGGER IF EXISTS on_auth_user_created_force_null_role ON public.profiles;
...
DROP TRIGGER IF EXISTS on_brand_offer ON public.product_applications;
...
DROP TRIGGER IF EXISTS on_moment_proposal ON public.moment_proposals;
...
DROP TRIGGER IF EXISTS on_product_application ON public.product_applications;
...
DROP TRIGGER IF EXISTS trigger_set_proposal_team_ids ON public.product_applications;
...
DROP POLICY IF EXISTS brand_deposits_admin_all ON public.brand_deposits;
...
DROP POLICY IF EXISTS brand_deposits_read_own ON public.brand_deposits;
...
DROP POLICY IF EXISTS brand_products_delete ON public.brand_products;
...
DROP POLICY IF EXISTS brand_products_insert ON public.brand_products;
...
DROP POLICY IF EXISTS brand_products_update ON public.brand_products;
...
DROP POLICY IF EXISTS brand_proposals_delete ON public.product_applications;
...
DROP POLICY IF EXISTS brand_update_moment_proposals ON public.moment_proposals;
...
DROP POLICY IF EXISTS brand_update_performance ON public.campaign_performance;
...
DROP POLICY IF EXISTS campaigns_delete_team ON public.campaigns;
...
DROP POLICY IF EXISTS campaigns_insert_team ON public.campaigns;
...
DROP POLICY IF EXISTS campaigns_update_team ON public.campaigns;
...
DROP POLICY IF EXISTS creator_insert_performance ON public.campaign_performance;
...
DROP POLICY IF EXISTS instagram_insert ON public.instagram_accounts;
...
DROP POLICY IF EXISTS instagram_select ON public.instagram_accounts;
...
DROP POLICY IF EXISTS instagram_update ON public.instagram_accounts;
...
DROP POLICY IF EXISTS manage_team_members_delete ON public.team_members;
...
DROP POLICY IF EXISTS manage_team_members_insert ON public.team_members;
...
DROP POLICY IF EXISTS manage_team_members_update ON public.team_members;
...
DROP POLICY IF EXISTS mcn_revenue_splits_insert ON public.mcn_revenue_splits;
...
DROP POLICY IF EXISTS mcn_revenue_splits_select ON public.mcn_revenue_splits;
...
DROP POLICY IF EXISTS mcn_revenue_splits_update ON public.mcn_revenue_splits;
...
DROP POLICY IF EXISTS moment_proposals_insert ON public.moment_proposals;
...
DROP POLICY IF EXISTS products_delete_team ON public.brand_products;
...
DROP POLICY IF EXISTS products_insert_team ON public.brand_products;
...
DROP POLICY IF EXISTS products_select_authenticated ON public.brand_products;
...
DROP POLICY IF EXISTS products_update_team ON public.brand_products;
...
DROP POLICY IF EXISTS select_own_performance ON public.campaign_performance;
