...
-- 크리에이터가 브랜드 제품을 보고 지원하는 테이블이므로
-- product_applications 로 이름을 변경합니다.
-- =====================================================
...
-- 3. submission_feedback 테이블: brand_proposal_id 컬럼명 변경
...
BEGIN
    ...
    FROM (
        SELECT
            ...
            -- Product Applications stats (renamed from brand_proposals)
        ...
        LEFT JOIN LATERAL (
            ...
            WHERE pa.influencer_id = tm.user_id
        ) pa_stats ON true
        LEFT JOIN LATERAL (
            SELECT
                COUNT(*) AS total_proposals,
...
-- 6. get_team_proposals RPC: proposal_type 값 변경
...
BEGIN
    ...
    FROM (
        -- Product Applications (formerly brand_proposals)
        SELECT
            pa.id,
            ...
            pa.content_type,
            pa.brand_condition_confirmed,
            pa.influencer_condition_confirmed,
            pa.contract_status,
            pa.delivery_status
        FROM public.product_applications pa
